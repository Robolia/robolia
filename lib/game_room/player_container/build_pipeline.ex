defmodule GameRoom.PlayerContainer.BuildPipeline do
  use Opus.Pipeline
  alias GameRoom.PlayerContainer.Register
  require Logger
  ...
  def clone_player_bot(%{game: game, player: player} = pipeline) do
    ...
    result = "git clone #{player.repository_clone_url} #{bot_local_storage_path}"
             |> run_cmd
  ...
  def start_bot_container(%{container_id: container_id} = pipeline) do
    result = "docker container start #{container_id}"
    |> run_cmd
  ...
  def copy_bot_to_container(%{
    ...
    result = "docker cp #{bot_local_storage_path}. #{container_id}:/app"
    |> run_cmd
  ...
  def prepare_bot_to_run(%{container_id: container_id} = pipeline) do
    result = "docker container exec #{container_id} mix compile"
    |> run_cmd
