defmodule Robolia.PlayerContainer do
  ...
  def run(%{
    ...
    Logger.debug("docker container exec #{container_id} #{bot_command}")

    "docker container exec #{container_id} #{bot_command}"
    |> run_cmd()
    |> runner_adapter.parse_bot_response()
  end
  ...
  def delete(%{game: %{id: game_id}, player: %{id: player_id}}) do
    ...
    run_cmd("docker rm #{container_id} --force")
    run_cmd("rm -rf #{bot_local_storage_path}")
