defmodule Robolia.Games.TicTacToes.MatchTest do
  use Robolia.DataCase
  alias Robolia.Games.TicTacToes.{Match, Queries, TicTacToeMoviment, TicTacToeMatch}
  alias Robolia.Repo
  import Mox
  ...
  describe "play/1" do
    ...
    test "returns a finished {:ok, match} when a winner move is found in the game", ctx do
      Robolia.PlayerContainerMock |> stub(:run, fn %{next_turn: turn} -> '#{turn}\n' end)
    ...
    test "declares the opponent the winner if current player makes an invalid moviment", ctx do
      Robolia.PlayerContainerMock |> stub(:run, fn %{next_turn: _} -> 'ERROR' end)
